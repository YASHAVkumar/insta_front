        ...
        <div className="container mt-3" >
            {postInfo
                ...
                <div className="margin-top-bottom container-sm px-4 w-50">
                    <div className="box box-shadow">
                        ...
                        {userInfo !== false && userInfo !== null
                            ? <div className="post-section">
                                ...
                                <NavLink to="#comment" style={{marginLeft:"20px"}}><span className="share-icon"><img src={CommentIcon} alt="Comment Icon" /></span></NavLink>
                        ...
                        <h6 className="box-text">Posted {moment(postInfo.createdAt).fromNow()} by <NavLink className="link" to={`/u/${username}`}>{username}</NavLink></h6>
                            ...
                            <div>
                              <br/><br/>
                               <div style={{display:"flex",justifyContent:"space-evenly"}}>
                                <h3><NavLink to={`/post/${params.match.params.id}/edit`} className="link btn btn-sm btn-outline-warning">Edit Post</NavLink></h3>
                                <h3 className="link btn btn-sm btn-outline-danger" onClick={DeletePost}>Delete Post</h3>
                                </div>
                            </div>
                            : null
                        }
                    ...
                    <hr/>
                    <div className="margin-top-bottom box box-shadow border p-3" id="comment">
                        <h5 className="box-title" style={{fontFamily:" cursive"}}>Comments</h5>
                        <div className="margin box box-shadow">
                            ...
                            &nbsp; <mark> {comment.comment} </mark> {moment(comment.createdAt).fromNow()}</p>
                            })}
                        </div>
                        {userInfo ?
                            <form onSubmit={addComment}>
                                <textarea className="form-control" rows="2" onChange={({ target: { value } }) => setInputComment(value)} placeholder="Enter Your Comments Here"></textarea>
                                <br /> <input className="btn-outline-primary btn btn-sm" type="submit" />
                            </form>
                            : null}
                    </div>
                </div>
