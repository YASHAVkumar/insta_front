...
const Register = () => {
    ...
    const register = (e) => {
        e.preventDefault();
        if (username.length >= 5 && (password === confirmation && password.length>=5&&password=="(?=.*\d)(?=.*[a-z])(?=.*[A-Z]).{8,}")) {
            const User = {
                username: username,
                password: password,
                email: email
           ...
           else
           {
              if(password!=confirmation)
               ...
               setMsg("password length must be greater than 5");
               else
               setMsg("password must contain one lower letter, upper letter, number");
    ...
    return (
        ...
        <div className="container-sm" style={{width:"450px"}}>
