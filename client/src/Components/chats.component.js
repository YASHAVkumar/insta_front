...
const Chats = ({ location }) => {
    ...
    const FilterUser = (e) => {
        ...
        for (let i = 0; i < li.length; i++) {
            let user = li[i];
            div = user.getElementsByTagName('td')[0];
 ...
 const [chosenEmoji, setChosenEmoji] = useState(null);
  ...
  const onEmojiClick = (event, emojiObject) => {
        console.log( emojiObject);
         var str=emojiObject.emoji;
                socket.emit('sendMessage', {str, from: userInfo._id, to: receiverId })
                setMessages(messages => [...messages, { chat:str, from: userInfo._id, to: receiverId }]);
                ...
                <div className="container">
                    ...
                    <NavLink to="/" className="link">Back to home page</NavLink></h3>
                        : <div className="margin box box-shadow">
                            ...
                            <div className="chats bg-dark">
                                <ScrollToBottom className="messages">
                                    {messages.map(message => {
                                        if (message.from === userInfo._id) {
                                            return <div key={message._id || message.chat} className="sentText"><h3  className="message text-warning">{message.chat}</h3></div>
                                        } else {
                                            return <div key={message._id || message.chat} className="receivedText"><h3 className="message text-success">{message.chat}</h3></div>
                                        }
                                    })}
                                </ScrollToBottom>
                                     ...
                                     <Picker onEmojiClick={onEmojiClick} />
                                      {chosenEmoji ? (
                                             <h1>You chose: {chosenEmoji.emoji}</h1>
                                           ) : (
                ...
                : <div className="container">
                   <div className="container">
                        <div className=" form-group">
                            ...
                            <input type="text" className="form-control" style={{width:"200px"}} placeholder="Search for username..." id="searchUsername" onChange={FilterUser} />
                        </div>
                          ...
                          <tbody>
                            {userList.map((user) => {
                                if (user !== userInfo.username) {
                                    return <tr key={user} className="user"><td  style={{textAlign:"center"}}
                                        onClick={() => window.location = `${window.location.pathname}?qto=${user}`}>{user}</td></tr>
                                }
                                else return null;
                            })}
                           </tbody>
