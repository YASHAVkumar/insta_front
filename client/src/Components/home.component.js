...
async function check_token() {
    ...
    await Axios.get(`${BACKEND_URL}/users`)
        .then(res => {
            (res.data).forEach(i => {
                if (i.token === token) {
                    {logged_in = i;console.log(logged_in);}
                }
            })
  ...
  var i=1;
    ...
    const GeneratePost = ({ post }) => {
        return <div key={post._id} className="box box-shadow margin-top-bottom container-sm px-4 w-50">
            <b><NavLink to={`/u/${post.creator}`} className="link fs-2 text text-wrap"><h5>{post.creator}</h5></NavLink></b>
            ...
            <div className="post-section">
                ...
                <NavLink to={`/post/${post._id}/#comment`} style={{marginLeft:"10px"}}><span className="share-icon"><img src={CommentIcon} alt="Comment Icon" /></span></NavLink>
            ...
            <h6 className="box-text"> {moment(post.createdAt).fromNow()}</h6>
    ...
    return (
        <div className="container">
         {/*
          <div style={{height:"40px",border:"2px solid blue",width:"50%",marginLeft:"150px"}}>
             <h3>Add Stories Coming Soon</h3>
          </div>
          */}

            <div className="home" style={{height:"500px",width:"70%",overflowY:"scroll",scrollBehavior:"smooth"}}>
                {posts.length !== 0 ? (
                    [posts.map((post) => {
            ...
            <div className="recommendation" style={{position:"relative",border:"2px solid black"}}>
                <div className="margin box box-shadow">
                    <h4 className="box-title">Suggestions For Your</h4>
                    <ul style={{listStyleType:"none"}}>
                        {newestUser.map((user) => {
                            if (user.username !== userInfo.username) {
                                return <li key={user._id}><NavLink className="link" to={`/u/${user.username}`}>{user.username}</NavLink>  <a href="#" style={{textDecoration:"none",float:"right"}}>Follow</a>{//(You may like this user {moment(user.createdAt).fromNow()})
                                }</li>
                            } else { return null; }
                        })}
                    </ul>
